...
string convertTimetoString(time_t date){
    ...
    char output[11];
    strftime(output, sizeof(output), "%Y-%m-%d", time);
    return output ;
}
	...
	public:
		Inventory(){
			...
			while (getline(file, fileData)) {
			    ...
			    time_t date = convertStringToTime(tokens[4]);
			    
			    items.emplace_back(name, category, quantity, barcode, date);
			}
			...
			for(auto item: items){
				if(item.isExpired()){
					cout<<setw(20)<<left<<index<<setw(20)<<left<<item.name<<setw(20)<<left<<item.barcode
						<<setw(20)<<left<<"Expired"<<setw(20)<<left<<convertTimetoString(item.expiryDate)<<endl;
				}else
				if(item.isExpiring()){
					cout<<setw(20)<<left<<index<<setw(20)<<left<<item.name<<setw(20)<<left<<item.barcode
						<<setw(20)<<left<<"Expiring"<<setw(20)<<left<<convertTimetoString(item.expiryDate)<<endl;
				}
				index++;
			}
		}
		...
		void Display(){
			...
			for(auto item:items){
				cout<<setw(20)<<left<<item.name<<setw(20)<<left<<item.category<<setw(20)<<left<<item.quantity
				    <<setw(20)<<left<<item.barcode<<setw(20)<<left<<convertTimetoString(item.expiryDate)<<endl;
			}
		}
